...
using WarehouseLogisticsEditor.WebApi.JWT;
using DevExpress.ExpressApp.Security.Authentication;
using DevExpress.ExpressApp.Security.Authentication.ClientServer;
using WarehouseLogisticsEditor.WebApi.Core;
using DevExpress.ExpressApp.AspNetCore.WebApi;

namespace WarehouseLogisticsEditor.WebApi;
...
public class Startup {
    ...
    public void ConfigureServices(IServiceCollection services) {
        ...
        services.AddXafAspNetCoreSecurity(Configuration, options => {
            ...
            options.UserType = typeof(WarehouseLogisticsEditor.Module.BusinessObjects.ApplicationUser);
            // ApplicationUserLoginInfo is only necessary for applications that use the ApplicationUser user type.
            // If you use PermissionPolicyUser or a custom user type, comment out the following line:
            options.UserLoginInfoType = typeof(WarehouseLogisticsEditor.Module.BusinessObjects.ApplicationUserLoginInfo);
            ...
            options.Events.OnAuthenticated = (externalAuthenticationContext) => {
                ...
                externalAuthenticationContext.Principal.Identity.AuthenticationType != SecurityDefaults.WindowsAuthentication && !(externalAuthenticationContext.Principal is WindowsPrincipal)) {
                    ...
                    object CreateApplicationUser(string userName, string providerUserId) {
                        if(objectSpace.FirstOrDefault<WarehouseLogisticsEditor.Module.BusinessObjects.ApplicationUser>(user => user.UserName == userName) != null) {
                            throw new ArgumentException($"The username ('{userName}') was already registered within the system");
                        }
                        var user = objectSpace.CreateObject<WarehouseLogisticsEditor.Module.BusinessObjects.ApplicationUser>();
                    ...
                    ISecurityUserLoginInfo FindUserLoginInfo(string loginProviderName, string providerUserId) {
                        return objectSpace.FirstOrDefault<WarehouseLogisticsEditor.Module.BusinessObjects.ApplicationUserLoginInfo>(userLoginInfo =>
        ...
        services.AddSwaggerGen(c => {
            ...
            c.SwaggerDoc("v1", new OpenApiInfo {
                Title = "WarehouseLogisticsEditor API",
                Version = "v1",
                Description = @"Use AddXafWebApi(options) in the WarehouseLogisticsEditor.WebApi\Startup.cs file to make Business Objects available in the Web API."
            });
                ...
                Flows = new OpenApiOAuthFlows() {
                    AuthorizationCode = new OpenApiOAuthFlow() {
                        ...
                        Scopes = new Dictionary<string, string> {
                            ...
                            { @"[Enter the scope name in the WarehouseLogisticsEditor.WebApi\Startup.cs file]", @"[Enter the scope description in the WarehouseLogisticsEditor.WebApi\Startup.cs file]" }
                        }
                    }
                }
            });
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env) {
        if(env.IsDevelopment()) {
            ...
            app.UseSwagger();
            app.UseSwaggerUI(c => {
                c.SwaggerEndpoint("/swagger/v1/swagger.json", "WarehouseLogisticsEditor WebApi v1");
